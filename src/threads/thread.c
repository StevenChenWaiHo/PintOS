...
static void thread_update_priority(struct thread *thread)
{
  thread->curr_priority = PRI_MAX - (thread->recent_cpu / 4) - (thread->nice * 2);
}
...
{
  return thread_mlfqs ? thread_current()->recent_cpu : 0;
}
